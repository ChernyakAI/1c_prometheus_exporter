...
Процедура ВыполнитьОтправкуМетрик() Экспорт
	...
	ОтправитьМетрики();
	ОценкаПроизводительности.ЗакончитьЗамерВремени("Экспорт метрик в Prometheus", ВремяНачалаЗамера);
...
Функция ПодготовитьТелоСообщения(НастройкиОтправки = Неопределено)
	...
	ОбщегоНазначенияКлиентСервер.ДополнитьМассив(СоставСообщения, ГруппаМетрикАпдексОбщий);
	ОбщегоНазначенияКлиентСервер.ДополнитьМассив(СоставСообщения, ГруппаМетрикАпдекс);
	ОбщегоНазначенияКлиентСервер.ДополнитьМассив(СоставСообщения, ГруппаМетрикКоличествоЗамеров);
	ОбщегоНазначенияКлиентСервер.ДополнитьМассив(СоставСообщения, ГруппаМетрикСреднее);
	ОбщегоНазначенияКлиентСервер.ДополнитьМассив(СоставСообщения, ГруппаМетрикМинимум);
	ОбщегоНазначенияКлиентСервер.ДополнитьМассив(СоставСообщения, ГруппаМетрикМаксимум);
...
Функция ВыбратьЗамеры()
	...
	ИнтервалСбора = Константы.пэ_ИнтервалСбораПоказателей.Получить();
	ИнтервалСбораСекунд = пэ_Служебный.ИнтервалСбораВСекундах(ИнтервалСбора);
	...
	|ИЗ
	|	Справочник.пэ_ПрофилиКлючевыхОпераций КАК пэ_ПрофилиКлючевыхОпераций
	|ГДЕ
	...
	ОценкаПроизводительности.ЗакончитьЗамерВремени("Экспорт метрик в Prometheus. Запрос данных.", ВремяНачалаЗамера);
